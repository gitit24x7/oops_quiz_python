...
function replaceSections(content, chapterKey, newSectionsObject) {
  const startStr = '"' + chapterKey + '": {';
  const startIdx = content.indexOf(startStr);
  if (startIdx === -1) return content;
  
  const sectionsMatch = content.indexOf('sections: [', startIdx);
  if (sectionsMatch === -1) return content;
  
  let openBrackets = 0;
  let sectionsEndIdx = -1;
  for (let i = sectionsMatch + 9; i < content.length; i++) { // start at '['
  ...
  if (sectionsEndIdx === -1) return content;
  
  const before = content.substring(0, sectionsMatch);
  const after = content.substring(sectionsEndIdx);
  const newStr = 'sections: ' + JSON.stringify(newSectionsObject, null, 6);
